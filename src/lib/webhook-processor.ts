...
/**
 * Schedule a delayed comment check by setting the scheduledCheckTime in the database
 */
async function scheduleCommentCheck(
  taskId: number,
  delayMs: number = 60000, // 60 seconds
): Promise<void> {
  const scheduledCheckTime = new Date(Date.now() + delayMs);
  console.log(
    `Scheduling comment check for task ${taskId} at ${scheduledCheckTime}`,
  );

  await db.julesTask.update({
    where: { id: taskId },
    data: { scheduledCheckTime },
  });
}
...
export async function processJulesLabelEvent(
  ...
  try {
    ...
    if (labelName === "jules" && action === "labeled") {
      ...
      await scheduleCommentCheck(task.id, delayMs);
